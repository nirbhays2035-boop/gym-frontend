  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="bg-card border border-border rounded-2xl p-8 shadow-sm">
        ...
        <p className="text-sm text-muted-foreground mb-7">Join GymOS as a member</p>
        ...
        <form onSubmit={handleSubmit((d) => register_.mutate(d))} className="space-y-5">
          ...
          {register_.error && (
            <div className="text-sm text-destructive bg-destructive/10 border border-destructive/20 rounded-lg px-3 py-2.5">
              {(register_.error as AxiosError<ApiResponse<unknown>>)?.response?.data?.message ?? 'Registration failed.'}
            </div>
          )}
